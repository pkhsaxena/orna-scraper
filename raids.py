...
def parse(content) -> bool:
    ...
    for raid_boss in raids:
        details = {}
        details['name'] = raid_boss.contents[1].contents[0]
        details['tier'] = raid_boss.contents[2].contents[0].replace('\u2605', '')
        details['img'] = raid_boss.contents[0].contents[0]['src']
        tag = {}
        tag[raid_boss['href'].split("/")[-2]] = details
...
if __name__ == "__main__":
    page = 1
    url = f'https://playorna.com/codex/raids/?p={page}'
    ...
    codex_classes = []
    with open('temp.dump', 'r', encoding='utf-8') as content, open('raids.json', 'w', encoding='utf-8') as codex:
