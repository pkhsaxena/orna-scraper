...
def parse(content) -> bool:
    ...
    monsters = soup.find_all('a', class_='codex-entries-entry')

    for mon in monsters:
        details = {}
        details['name'] = mon.contents[1].contents[0]
        details['family'] = mon.contents[2].contents[0]
        details['tier'] = mon.contents[3].contents[0].replace('\u2605', '')
        details['img'] = mon.contents[0].contents[0]['src']
        tag = {}
        tag[mon['href'].split("/")[-2]] = details
...
if __name__ == "__main__":
    page = 1
    url = f'https://playorna.com/codex/monsters/?p={page}'
    ...
    codex_classes = []
    with open('temp.dump', 'r', encoding='utf-8') as content, open('monsters.json', 'w', encoding='utf-8') as codex:
